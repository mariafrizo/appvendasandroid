...
class MainApp(App):
    ...
    def carregar_infos_usuario(self):
        try:
            ...
            requisicao = requests.get(f"https://aplicativovendashash-95608-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}")
    ...
    def mudar_foto_perfil(self, foto, *args):
        ...
        info = f'{{"avatar": "{foto}"}}'
        requisicao = requests.patch(f'https://aplicativovendashash-95608-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}', data= info)
    ...
    def adicionar_vendedor(self, id_vendedor_adicionado):
        ...
        if requisicao_dic == {}:
            ...
            if id_vendedor_adicionado in equipe:
                ...
                requests.patch(f"https://aplicativovendashash-95608-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}", data=info)
    ...
    def adicionar_venda(self):
        ...
        if cliente and produto and unidade and preco and quantidade and (type(preco) == float) and (type(quantidade)==float):
            ...
            requests.post(f"https://aplicativovendashash-95608-default-rtdb.firebaseio.com/{self.local_id}/vendas.json?auth={self.id_token}", data=info)

            banner = BannerVenda(cliente=cliente, produto=produto, foto_cliente=foto_cliente, foto_produto=foto_produto, data=data, preco=preco, unidade=unidade, quantidade=quantidade)
            pagina_homepage = self.root.ids["homepage"]
            lista_vendas = pagina_homepage.ids["lista_vendas"]
            lista_vendas.add_widget(banner)

            requisicao = requests.get(f"https://aplicativovendashash-95608-default-rtdb.firebaseio.com/{self.local_id}/total_vendas.json?auth={self.id_token}")
            total_vendas = float(requisicao.json())
            total_vendas += preco
            info = f'{{"total_vendas": "{total_vendas}"}}'
            requests.patch(f"https://aplicativovendashash-95608-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}",
                           data=info)
            homepage = self.root.ids["homepage"]
            homepage.ids["label_total_vendas"].text = f"[color=#000000]Total de Vendas:[/color] [b]R${total_vendas}[/b]"
            self.mudar_tela("homepage")
    ...
    def carregar_todas_vendas(self):
        ...
        requisicao = requests.get(f'https://aplicativovendashash-95608-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"')
        requisicao_dic = requisicao.json()
        print("requisicao dic de preencherj a pagina todasvendaspage: ", requisicao_dic)
    ...
    def carregar_vendas_vendedor(self, dic_info_vendedor, *args):
        print("carregar vendas vendedor")
        try:
            vendas = dic_info_vendedor["vendas"]
            print("vendas do carregar vendas vendedor: ", vendas)
            pagina_vendasoutrovendedor = self.root.ids["vendasoutrovendedorpage"]
            lista_vendas = pagina_vendasoutrovendedor.ids["lista_vendas"]
            print("lista vendas do vendasoutrovendedorpageeee: ", lista_vendas)

            # limpar vendas anteriores
            print("limpar vendas anteriores")
            for item in list(lista_vendas.children):
                lista_vendas.remove_widget(item)

            for id_venda in vendas:
                print("id venda de carregar_vendas_vendedor: ", id_venda)
                venda = vendas[id_venda]
                print("venda de carregar_vendas_vendedor: ", venda)
